    ...
    def load_config(self, path):
        if os.path.exists(path):
            with open(path, "r") as f:
                ...
                self.console_output = data.get("console_output", False)
...
class LogReader:
    ...
    def read(self, level=None, start_time=None, end_time=None):
        ...
        with open(self.log_file, "r", encoding="utf-8") as f:
            for line in f:
                ...
                lvl = parts[2].strip("[]")
                if level and LogLevel[lvl] < level:
                    continue
...
class Report:
    ...
    def export(self, filename):
        with open(filename, "w", encoding="utf-8") as f:
            if self.format == "html":
                f.write("<html><body style='background:{};color:{};font-size:{}px;'>".format(
                    self.style.bg_color, self.style.font_color, self.style.font_size))
                f.write(f"<h1>{self.header}</h1>")
                for name, content in self.sections:
                    f.write(f"<h2>{name}</h2><p>{content}</p>")
                f.write(f"<footer>{self.footer}</footer></body></html>")
...
class IReportBuilder:
    ...
    def add_section(self, section_name, section_content): pass
...
class TextReportBuilder(IReportBuilder):
    ...
    def set_header(self, header): self.report.header = header
    def set_content(self, content): self.report.content = content
    def set_footer(self, footer): self.report.footer = footer
    def add_section(self, section_name, section_content): self.report.sections.append((section_name, section_content))
    def set_style(self, style): self.report.style = style
...
class Character:
    ...
    def clone(self):
        return Character(
            ...
            self.weapon.clone(),
            self.armor.clone(),
            [skill.clone() for skill in self.skills]
        )
